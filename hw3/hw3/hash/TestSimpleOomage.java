...
public class TestSimpleOomage {
    ...
    public void testHashCodePerfect() {
         ...
         */
        List<SimpleOomage> allObjects = new ArrayList<>();
        Set<Integer> allHashCodes = new HashSet<>();
        for (int r = 0; r < 256; r += 5) {
            for (int g = 0; g < 256; g += 5) {
                for (int b = 0; b < 256; b += 5) {
                    allObjects.add(new SimpleOomage(r, g, b));
                }
            }
        }
        for (SimpleOomage so : allObjects) {
            allHashCodes.add(so.hashCode());
        }
        assertEquals(allObjects.size(), allHashCodes.size());
    }
